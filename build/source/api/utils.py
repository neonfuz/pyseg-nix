...
class Log:
    ...
    def log_groundseg(text):
        ...
        try:
            ...
            os.system("mkdir -p /var/lib/groundseg/logs")

            # current log file
            current_logfile = f"{datetime.now().strftime('%Y-%m')}.log"

            # move legacy logfile to new directory
            if os.path.isfile(f"/var/lib/groundseg/groundseg.log"):
                os.system(f"mv /var/lib/groundseg/groundseg.log /var/lib/groundseg/logs/{current_logfile}")

            # write to logfile
            with open(f"/var/lib/groundseg/logs/{current_logfile}", "a") as log:
